								...
								<div className="img-container text-center" style={{ width: window.innerWidth - 370 + 'px', marginTop: '120px', transition: 0 + 's' }}>
									<div className="img-block" >
										{(states.interval === true) ?
											(count.toString() === x.work_id) ?
												<div>
													<h2 style={{ marginLeft: left + 'px', transition: trans + 's', color: '#' + getRandomArbitrary(100000, 999999), zIndex: 0, position: 'relative' }} className="descript" ><div>{x.work_name}</div></h2>
													<img style={{ marginLeft: left + 'px', transition: trans + 's', height: window.innerHeight - 300 + 'px', zIndex: 0, position: 'relative' }} src={`${inital_base_url}/img/slider/${x.work_img}`} alt={x.work_name} />
												</div>
												: ''
											: (states.counter.toString() === x.work_id) ?
												<div>
